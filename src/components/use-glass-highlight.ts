...
export const useGlassHighlight = ({
  ...
  const onPointerEnter = (e) => {
    ...
    // const lightColor = "var(--color-sf-hover-glass)";
    const lightColor = "color(from rgb(255, 255, 255) srgb 8 8 8)";
    // color(from rgb(255, 255, 255) srgb 8 8 8);
    // lightEl.style.backgroundImage = `radial-gradient(circle at center, color-mix(in oklab, ${lightColor} ${
    //   opacity * 100
    // }%, transparent), transparent 50%)`;
    lightEl.style.backgroundColor = lightColor; //`radial-gradient(circle at center, ${lightColor}, transparent 50%)`;
